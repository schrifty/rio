...
class InviteTest < ActiveSupport::TestCase
  ...
  test "that you can't create an invite with a duplicate id" do
    assert_raises ActiveRecord::RecordNotUnique do
      Invite.new(:id => 1, :tenant_id => 1, :recipient_email => "jdoe@gmail.com").save!
    end
  end
  ...
  test "should not save with an invalid tenant id" do
    invite = Invite.new({:tenant_id => 999, :recipient_email => "jdoe@gmail.com"})
    assert !invite.save
  end

  test "should not save without a recipient email" do
    invite = Invite.new({:tenant_id => 1})
    assert !invite.save
  end
end
